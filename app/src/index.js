...
const App = {
  ...
  captureFile:function() {//加载图片到ipfs
    var reader;
  ...
  setGoods: async function(){//商家上架商品
    ...
    try{
      ...
      //console.log(isBuy2);
      await setGoods(goodsId4, priceGoods2, avaGoods, isBuy2).send({from: this.account});
      this.refresh("parentBal", "teenBal", "teenLim", "sellerMoney", this.account);
    }catch(err){
  ...
  getallGoods: async function(){//查询某一商家某一商品信息
    try{
      ...
      console.log(id + "       " + priceGoods + "       " + avaGoods + "       " + saleGoods);
    }catch(err){
